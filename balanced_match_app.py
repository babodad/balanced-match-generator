...
players_input = st.text_input("Spielerliste (kommagetrennt)", "Babo, Lenny, Gugax, Nessie")
decks_input = st.text_input("Deckliste (kommagetrennt)", "White, Blue, Black, Red, Green, Colorless")
...
if st.button("Turnierplan generieren"):
    ...
    for i, (p1, p2) in enumerate(pairings, 1):
        ...
        for d1, d2 in deck_combos:
            key = f"{p1}-{p2}-{d1}-{d2}"
            if (
                d1 not in player_decks[p1] and
                d2 not in player_decks[p2] and
                deck_usage[d1] < 4 and
                deck_usage[d2] < 4 and
                key not in used_combos
            ):
                result.append({
                    "Runde": i,
                    "Spieler 1": p1,
